...
function DFP(x::Array{Float64}, n::Int64)
        tol = 1E-4
        B = I
        for k = 1:n
            gradien = grad(x)
...
function BFP_rank1(x::Array{Float64}, n::Int64)
        tol = 1E-4
        B = I
        for k = 1:n
            gradien = grad(x)
            ...
            if norma < tol
                ...
                norma1 = norm(grad1)
                    if norma1 < tol
                        println("X otimo é: ", x1)
                        break
                    else
                        g = grad1 .- gradien
                        s = x1 .- x
                        deltaB = ((s - B * g) * transpose((s - B * g)))/(transpose((s - B * g)) * g)
                        B = B + deltaB
                    end
            end
